...
from algo.pnps_cot import calculate_ps_pn, test_original_metrics  # Adjust this import if needed
...
def get_metrics_for_line(json_line: str, prompt_based: bool) -> dict:
    ...
    choice = extract_field(data, ["choices"])  # 可能是 choices，而不是 choice
    model_cot = extract_field(data, ["_response"])
    ground_truth = extract_field(data, ["answerKey"])

    # 提取 choices 中的标签和文本
    choice_labels = [f"{label}.{text}" for label, text in zip(choice["label"], choice["text"])]

    # 结合 question 和 choice_labels
    formatted_choice = " ".join(choice_labels)

    # Pass the boolean flag to calculate_ps_pn by converting it into the expected type
    results = test_original_metrics(
        query=question + " " + formatted_choice,  # 确保拼接正确
        response=model_cot,
