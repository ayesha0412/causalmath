...
for dataset in "${datasets[@]}"; do
    input_file="$base_dir/$dataset/test_qwen_with_qwen_rollouts_results_commonsenseqa.jsonl"
    
    if [[ -f "$input_file" ]]; then
        echo "Processing dataset '$dataset'..."

        # Output paths
        np_output="$base_dir/$dataset/qwen_commonsenseqa_after_pn_np.jsonl"
        pb_output="$base_dir/$dataset/qwen_commonsenseqa_after_pn_pb.jsonl"
